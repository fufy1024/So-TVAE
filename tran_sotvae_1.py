...
from coa_MAC1 import coa_MAC #!!!!!!!!!!!!!!!!!!!原coa_MAC1
...
c_means, c_sigma=init_clusters(num_clusters=opt.z_class, latent_size=opt.latent_size,c_m_file=f'./cluster_means_class{opt.z_class}.pickle')
...
data_path='/mnt/data10t/bakuphome20210617/ffy2020/ffy2020/VC/dataset_bilibili/'
...
train_path, test_path, dev_path = data_path + f'train-context-skep10.json', data_path + f'test-candidate-skep10.json', data_path + f'dev-candidate-skep10.json'
...
class Model(nn.Module):
    ...
    def __init__(self, n_emb, n_hidden, vocab_size, dropout, d_ff, n_head, n_block):
        ...
        #co_attention
        self.coa=coa_MAC(out_size=opt.z_class,LAYER=3,HIDDEN_SIZE=n_hidden,FLAT_MLP_SIZE=n_hidden,FLAT_GLIMPSES=1,DROPOUT_R=dropout,FLAT_OUT_SIZE=1024)#从这里改变层数
    ...
    def decode(self,  x,x_out,y_out,x_out1,y_out1,z_emb, mask,label_emb):
        ...
        out = self.output_layer(out)##[ 90, 19, 30005],需要改维度设置
        return out
    ...
    def forward(self, label,label_mean, c_sigma,X, Y, T):
        #Y:[64, 20] comment ; T:[64, 100] context，X:[64, 5, 512] image
        self.T=T
    ...
    def generate(self, z, label_i,x_out1, y_out1):#label：[1,3]，z:[1,64]
        ...
        for i_gen in range(opt.max_len):
            ...
            out = self.decode_test(ys, x_out,y_out ,z_emb,
                              Variable(subsequent_mask(ys.size(0), ys.size(1))).cuda(),label_emb)     
            prob = out[:, -1]#[1, 30005]
...
def train():
    ...
    for i in range(opt.epoch):
        ...
        for it,batch in enumerate(train_batch):
            model.zero_grad()
            #Y:[64, 20]为"comment"; T:[64, 100]为"context"，X为image:[64, 5, 512]
            X, Y, T ,label= batch 
            ...
            loss = loss_1 + loss_vae*2+loss_z*1.5
...
def test_diversity(test_set, model):
    ...
    score, _=evaluation.compute_scores(ref, comemnt_gen)
    self_bleu, _ = Bleu().compute_score(ref_self, gen_self)

    print('score:',score)  
    print('self_bleu',self_bleu)

    ref_bleu, _ = Bleu().compute_score(ref, comemnt_gen)
    print('ref_bleu:',ref_bleu)  
...
if __name__ == '__main__':
    ...
    if opt.mode == 'train': #train
        train()
    elif opt.mode == 'test_div': # test diversity
