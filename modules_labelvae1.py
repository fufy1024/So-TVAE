...
class CommentDecoder(nn.Module):
    ...
    def forward(self, x,x_con,x_img,z_emb, mask,label_emb):
        for layer in self.layers:
            x = layer(x, x_con,x_img, z_emb,mask,label_emb)
        return self.norm(x)
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, d_model, d_ff, n_head, dropout):
        ...
        self.coa_con_attn = MultiHeadedAttention(n_head, d_model)
        self.coa_img_attn = MultiHeadedAttention(n_head, d_model)
        self.z_attn = MultiHeadedAttention(n_head, d_model)
        #label
        self.label_attn = MultiHeadedAttention(n_head, d_model)
    ...
    def forward(self, x, x_con,x_img,z_emb, mask,label_emb):
        ...
        #label:
        x = self.sublayer[2](x, lambda x: self.label_attn(x, label_emb, label_emb))

        #x = self.sublayer[6](x, lambda x: self.video_attn(x, m1, m1))
        #x = self.sublayer[7](x, lambda x: self.text_attn(x, m2, m2))
        #coattention+VAE
        x = self.sublayer[4](x, lambda x: self.coa_con_attn(x, x_con, x_con))
        x = self.sublayer[5](x, lambda x: self.coa_img_attn(x, x_img, x_img))
        x = self.sublayer[1](x, lambda x: self.z_attn(x, z_emb, z_emb))
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):#0.1
...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, d_model, d_ff, dropout=0.1):#0.1
...
class z_Decoder(nn.Module):
    ...
    def __init__(self, d_model, d_ff, n_head, dropout, n_block):
        ...
        self.layers = nn.ModuleList([z_DecoderBlock(d_model, d_ff, n_head, dropout) for _ in range(n_block)])
        self.norm = LayerNorm(d_model)

    def forward(self,  x, m2,x_con,x_img):
        for layer in self.layers:
            x = layer( x, m2, x_con,x_img)
        return self.norm(x)    


class z_DecoderBlock(nn.Module):

    def __init__(self, d_model, d_ff, n_head, dropout):
        super(z_DecoderBlock, self).__init__()
        self.video_attn = MultiHeadedAttention(n_head, d_model)
        self.text_attn = MultiHeadedAttention(n_head, d_model)
        self.coa_con_attn = MultiHeadedAttention(n_head, d_model)
        self.coa_img_attn = MultiHeadedAttention(n_head, d_model)
        self.feed_forward = PositionwiseFeedForward(d_model, d_ff, dropout)
        self.sublayer = nn.ModuleList([SublayerConnection(d_model, dropout) for _ in range(5)])

    def forward(self, x, m2,x_con,x_img):
        x = self.sublayer[1](x, lambda x: self.video_attn(x, x, x))
        x = self.sublayer[2](x, lambda x: self.text_attn(x, m2, m2))
        #coattention+VAE
        x = self.sublayer[0](x, lambda x: self.coa_con_attn(x, x_con, x_con))
        x = self.sublayer[4](x, lambda x: self.coa_img_attn(x, x_img, x_img))
...
class zBlock(nn.Module):
    def __init__(self, d_model, d_ff, n_head, dropout):
        ...
        self.self_attn = MultiHeadedAttention(n_head, d_model)
        self.else_attn = MultiHeadedAttention(n_head, d_model)
        self.feed_forward = PositionwiseFeedForward(d_model, d_ff, dropout)
        self.sublayer = nn.ModuleList([SublayerConnection(d_model, dropout) for _ in range(3)])

    def forward(self, x, m):#x对应label_emb，m对应GT_emb
        x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x))
        x = self.sublayer[1](x, lambda x: self.else_attn(x, m, m))
        return self.sublayer[2](x, self.feed_forward)

class zEncoder(nn.Module):
    def __init__(self, d_model, d_ff, n_head, dropout, n_block):
        super(zEncoder, self).__init__()
        self.layers = nn.ModuleList([zBlock(d_model, d_ff, n_head, dropout) for _ in range(n_block)])
        self.norm = LayerNorm(d_model)
